...
void volumeFog_float(
	...
	Samples = distanceToStart < MeshDistance ? min(Samples, 100) : 0; // 50 will be the maximum samples, you can change this value if needed
	...
	for (int i = 0; i < Samples; i++) {
		...
		if (length(vectorToAdd) < MinDistance + random) {
			Fog -= 0.02;
			continue;
		}
